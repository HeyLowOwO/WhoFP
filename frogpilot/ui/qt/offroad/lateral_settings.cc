...
FrogPilotLateralPanel::FrogPilotLateralPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> lateralToggles {
    ...
    {"SteerKI", parent->steerKi != 0 ? QString(tr("Ki Factor (Default: %1)")).arg(QString::number(parent->steerKi, 'f', 2)) : tr("Ki Factor"), tr("<b>How quickly openpilot corrects persistent steering errors.</b> Higher corrects faster but may overshoot; lower is smoother but slower. Default: 0.3."), ""},
  ...
  for (const auto &[param, title, desc, icon] : lateralToggles) {
    ...
    if (param == "AdvancedLateralTune") {
      ...
      lateralToggle = new FrogPilotParamValueButtonControl(param, title, desc, icon, 0.1, 0.7, QString(), std::map<float, QString>(), 0.01, false, {}, steerKiButton, false, false);
  ...
  QObject::connect(steerKiToggle, &FrogPilotParamValueButtonControl::buttonClicked, [parent, this]() {
    params.putFloat("SteerKI", parent->steerKi);
...
void FrogPilotLateralPanel::showEvent(QShowEvent *event) {
  ...
  steerKiToggle->setTitle(QString(tr("Ki Factor (Default: %1)")).arg(QString::number(parent->steerKi, 'f', 2)));
  steerKiToggle->updateControl(0.1, 0.7);
