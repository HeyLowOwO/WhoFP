...
def get_action_from_model(model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
                          lat_action_t: float, long_action_t: float, v_ego: float, use_curvature_from_plan: bool,
    ...
    if car_state is not None:
        ...
        if not blinker_on:
            ...
            state['left_cooldown'] = False
            state['right_cooldown'] = False
    ...
    if car_state is not None:
        blinker_on = car_state.leftBlinker or car_state.rightBlinker
        if blinker_on and v_ego < 14 and turn_detected and abs(steering_angle) > 40:
            blinker_turn = True
...
def main(demo=False):
  ...
  while True:
    ...
    if model_output is not None:
      ...
      action = get_action_from_model(model_output, prev_action, lat_delay + DT_MDL, long_delay + DT_MDL, v_ego, use_curvature_from_plan, sm["carState"].steeringAngleDeg, sm["carState"])
      prev_action = action
