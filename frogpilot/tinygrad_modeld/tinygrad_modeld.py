...
def get_action_from_model(model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
    ...
    if not hasattr(get_action_from_model, 'blinker_state'):
        get_action_from_model.blinker_state = {
            ...
            'event_counter': 0,
            'left_cooldown': False,   # NEW: Prevent mid-turn reactivation
            'right_cooldown': False,  # NEW: Prevent mid-turn reactivation
        }
    ...
    if car_state is not None:
        ...
        if state['left_active']:  # CHANGED: Only check when active
            if abs(steering_angle) > 160 or (abs(steering_angle) > 105 and turn_completing):  
