...
from openpilot.selfdrive.car.hyundai.values import (
  HyundaiFlags, CAR, DBC, CANFD_CAR, CAMERA_SCC_CAR,
  CANFD_RADAR_SCC_CAR, CANFD_UNSUPPORTED_LONGITUDINAL_CAR,
...
from openpilot.selfdrive.car.disable_ecu import disable_ecu, ecu_log  # FP10 BACKPORT
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs, frogpilot_toggles):
    ...
    # --- Longitudinal ---
    if candidate in CANFD_CAR:
      ret.experimentalLongitudinalAvailable = candidate not in (
  ...
  def init(CP, logcan, sendcan):
    # FP10 BACKPORT: improved ECU disable handling
    if CP.openpilotLongitudinalControl and not (
      ...
      disable_ecu(logcan, sendcan, bus=bus, addr=addr)

    if CP.flags & HyundaiFlags.ENABLE_BLINKERS:
      disable_ecu(logcan, sendcan, bus=CanBus(CP).ECAN, addr=0x7B1)
